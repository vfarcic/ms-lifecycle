...
node("cd") {
    ...
    stage "deploy"
    withEnv(envs) {
        sh "docker-flow -p go-demo --flow=deploy"
    }
    ...
    def tests = [:]
    addresses.each {
        tests[it.ServiceID] = { sh "HOST_IP=${it.ServiceAddress}:${it.ServicePort} docker-compose -f docker-compose-test.yml run --rm production" }
    }
    parallel tests
